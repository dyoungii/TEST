...
void IncaKoinGUI::createActions()
{
...
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/IncaKoin_testnet"));
#endif
            if(trayIcon)
            {
...
void IncaKoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void IncaKoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("IncaKoin client"));
...
#ifndef Q_OS_MAC
void IncaKoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void IncaKoinGUI::optionsClicked()
{
...
void IncaKoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to IncaKoin network", "", count));
}

void IncaKoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void IncaKoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void IncaKoinGUI::changeEvent(QEvent *e)
{
...
void IncaKoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void IncaKoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                IncaKoinUnits::formatWithUnit(IncaKoinUnits::BTC, nFeeRequired));
...
void IncaKoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(IncaKoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}

void IncaKoinGUI::gotoOverviewPage()
{
...
void IncaKoinGUI::gotoHistoryPage()
{
...
void IncaKoinGUI::gotoAddressBookPage()
{
...
void IncaKoinGUI::gotoBlockBrowser(QString transactionId)
{
...
void IncaKoinGUI::gotoReceiveCoinsPage()
{
...
void IncaKoinGUI::gotoSendCoinsPage()
{
...
void IncaKoinGUI::gotoSignMessageTab(QString addr)
{
...
void IncaKoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void IncaKoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void IncaKoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid IncaKoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void IncaKoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void IncaKoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void IncaKoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void IncaKoinGUI::changePassphrase()
{
